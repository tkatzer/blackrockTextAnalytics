...
stopwords <- append(stopwords("en"), c("january", "february", "march", "april", "may", "june", "july", "august","september", "october", "november", "december"))
...
a1 <- lexiscorpus$documents %>% separate('date', into=c("year","month","day"), sep = "-")
lexiscorpus$documents[,3]<-a1$year
# TF vom ersten Artikel
artikelneu <- which(docvars(lexiscorpus, 'V3') == "2019")
...
Ignaural_2019 <- textstat_keyness(Ignaural, target = which(docvars(Ignaural, 'V3') == "2019"))
##alle Texte bis jetzt sind aus 2019 :D mehr herunterladen
